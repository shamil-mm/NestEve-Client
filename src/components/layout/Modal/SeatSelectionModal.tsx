...
import { generateSeating } from "../../../utils/generateSeats/generateSeating";
...
const SeatSelectionModal: React.FC<SeatSelectionModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (cTicket && cTicket.selectedSeats && cTicket.selectedSeats.length > 0) {
  ...
  useEffect(() => {
    async function fetchBookedTickets() {
      try {
        ...
        const bookedSeats = res?.data?.selectedSeats;
        const layout = generateSeating(event.layoutConfig, ticket.name, bookedSeats);
  ...
  const handleSeatClick = (seat: any) => {
    ...
    if (isSelected) {
      ...
      setSelectedSeats((prev) => {
        if (prev.length >= ticket.quantity) return prev;
        return [...prev, seat];
      });
    }
  };
  ...
  const totalPrice = selectedSeats.reduce((total, seat) => {
    const category = event.layoutConfig.categories.find(
  ...
  const renderSeat = (seat: any, seatIdx: number) => {
    // const category = event.layoutConfig.categories.find(
    //   (cat) => cat._id === seat.category
    // );
    const isSelected = selectedSeats.some((s) => s.id === seat.id);

    if (!seat.isVisible) return <div key={seatIdx} className="w-6 h-6" />;

    return (
      <button
        key={seatIdx}
        className={`w-6 h-6 flex items-center justify-center border-blue-700 border-1 bg-black ${seat.status === "taken"
            ? "cursor-not-allowed border-red-600 font-bold"
            : isSelected
              ? "border-green-400 border-3 font-bold"
              : ""
          }`}
  ...
  const renderLayout = () => {
    const style = event.layoutConfig.seatStyle;

    if (style === "banquet") {
  return (
    <div className=" grid grid-cols-2 gap-6 justify-center">
      {filteredRows.map((row, rowIdx) => (
        <div key={rowIdx} className="relative left-30 w-40 h-40">
         
          <div className="absolute top-1/2 left-1/2 w-16 h-16 -translate-x-1/2 -translate-y-1/2 bg-blue-600 text-white rounded-full flex items-center justify-center shadow-md text-sm font-semibold">
            Table {row.id}
          </div>

         
          {row.seats.map((seat, seatIdx) => {
            const angle = (360 / row.seats.length) * seatIdx;
            const radius = 70;
    ...
    if (style === "classroom") {
      ...
      return (
        <div className="flex flex-col items-center gap-4">
          {filteredRows.map((row, rowIdx) => (
            <div key={rowIdx} className="flex flex-col items-center mb-4">
              <div className="w-full h-1 bg-gray-500 mt-1 rounded-full" />
              <br/>
              <div className="flex items-center justify-center gap-2">
                <span className="w-6 text-center font-bold mr-2">{row.id}</span>
                <div className="flex gap-2">{row.seats.map((seat, idx) => renderSeat(seat, idx))}</div>
              </div>
        ...
        {filteredRows.map((row, rowIdx) => (
          <div key={rowIdx} className="flex items-center mb-2 w-full justify-center">
            ...
            <div className="flex gap-1">
              {row.seats.map((seat, seatIdx) => renderSeat(seat, seatIdx))}
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="w-full bg-black/70 border-1 border-white text-white p-4 h-100vh">
      ...
      <div className="flex justify-center gap-3 mb-4 text-xs">
        <Legend color="border-blue-600" label="Available" />
        <Legend color="border-green-500" label="Selected" />
        <Legend color="border-red-500" label="Taken" />
      </div>
      ...
      <button
        ...
        className={`w-full py-2 font-bold bg-black border-1 border-blue-700 ${selectedSeats.length > 0 ? "" : "cursor-not-allowed"
          }`}
...
const Legend = ({ color, label }: { color: string; label: string }) => (
  <div className="flex items-center">
    <div className={`w-3 h-3 bg-black mr-1 border-1 ${color} rounded`}></div>
    <span>{label}</span>
  </div>
);
