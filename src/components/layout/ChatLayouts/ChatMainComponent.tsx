  ...
  return () => {
    ...
    useEffect(()=>{
       async function FetchChats(){
        console.log('fetchchats function is working')
         if(singleChat){
    ...
    useEffect(()=>{
      const handleRecieveMessage=async(message:any)=>{
        ...
        const realTimeformatedData=await formateChat([message],userId as string)
        console.log('real time data',realTimeformatedData)
            if(realTimeformatedData){
              setMessages((prev)=>[...prev,...realTimeformatedData])
              setTimeout(scrolltoBottom,100)
            }
      }
    ...
    useEffect(()=>{
      socket.on('showTyping',(senderId:string)=>{
        console.log('Received showTyping from:', senderId)
         setTimeout(scrolltoBottom,100)
        setTypingUser(senderId)
      })
      socket.on('hideTyping',(senderId:string)=>{
        console.log('Received hideTyping from:', senderId)
...
//   navigate(`/video-call?roomID=${singleChat._id}`)
//  }
    ...
    <div className="flex-1 flex flex-col">
        ...
        >
          ...
          <div className="space-y-4 max-w-4xl mx-auto">
            {messages.map((msg) => (
              <div onClick={()=>handleReplay(msg)} key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
      ...
      cancel
        ...
        <div className="bg-black p-4 border-t border-gray-700">
          <div className="flex items-center space-x-3 max-w-4xl mx-auto">
            <Paperclip className="w-5 h-5 text-gray-400 cursor-pointer hover:text-gray-300" />
