...
const Organizersidebar:React.FC<{ handlePageCB: (value:string) => void; }> = ({handlePageCB}) => {
  ...
  return (
    <div className="w-full sm:w-64 md:w-72 lg:w-80 h-auto bg-black/50 text-white border-blue-700 border-2 mb-4 sm:mb-0">
        <div className="p-3 sm:p-4">
          <nav>
            <ul className="space-y-4 sm:space-y-6 md:space-y-8 lg:space-y-10">

            {OrganizerOptions.map((option) => (
                  <li onClick={option.onClick} key={option.label} className="flex items-center p-2 text-indigo-500 cursor-pointer hover:bg-black/30 transition rounded">
