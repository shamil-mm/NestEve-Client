...
const EventDetailView:React.FC<EventDetailViewProps> = ({event,close}) => {
    ...
    return (
        ...
        &larr; back
          ...
          <div className="flex  justify-end gap-2 ml-auto p-2 m-2">
        <button className="p-1 bg-black/40 rounded-md hover:bg-black/60 transition ">
        <Pencil />
        </button>
        <button className="p-1 bg-black/40 rounded-md hover:bg-black/60 transition ">
        <Trash2 color='red' />
        ...
        />
          ...
          {/* Event Details Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-6  border border-white">
            {/* Left Column */}
            <div className="space-y-4">
              ...
              <div>
                <h2 className="text-sm text-white uppercase tracking-wider">Start Time</h2>
                <div className="flex items-center mt-1">
            ...
            <div className="space-y-4">
              ...
              <div>
                <h2 className="text-sm text-white uppercase tracking-wider">Seats</h2>
                <div className="flex items-center mt-1">
      ...
      event.ticketTypes.map((ticket, idx) => (
        <div key={idx} className="flex justify-between items-center text-sm">
          ...
          <p>${ticket.price }</p>
              ...
              {/* <div>
                <h2 className="text-sm text-white uppercase tracking-wider">Ticket charge</h2>
                <div className="flex items-center mt-1">
                  <DollarSign className="h-4 w-4 mr-2" />
                  <p>{event.is_seated===false?event?.ticketTypes[0].price: Math.min(...event.layoutConfig.categories.map((value)=>value.price)).toString() }</p>
                </div>
              </div> */}
